...
module.exports = {
    ...
    updateUser: (req, res) => {
        ...
        // Pass only the fields that exist in the request body
...
assignChildToParent: async (req, res) => {
    try {
      ...
      if (!parent_id || !child_id) {
        return res.status(400).json({
          success: false,
          message: "❌ Parent ID et Enfant ID sont requis",
        });
      }
  ...
  unassignChildFromParent: async (req, res) => {
    try {
      const { child_id, parent_id } = req.params;

      if (!child_id || !parent_id) {
        return res.status(400).json({
          success: false,
          message: "❌ IDs parent et enfant requis",
        });
      }

      await unassignParentChildRelation(parent_id, child_id);
      return res.status(200).json({
        success: true,
        message: "✅ Enfant désaffecté avec succès",
      });
    } catch (error) {
      console.error(error);
      return res.status(500).json({
   ...
   // user.controller.js
   getAllParentChild : (req, res) => {
  getAllParentChildRelations((error, results) => {
        if (error) {
            return res.status(500).json({
                success: false,
                message: "Erreur lors de la récupération des relations parent-enfant.",
                error: error.message
            });
        }
        return res.status(200).json({
